...
VS_OUTPUT main( VS_INPUT VSInput )
{
...
#if ( ( BUMPMAP || PHONG ) && USE_SPE ) || SHADOWMAP
	VSOutput.V_to_Eye_ShadowMap2PosZ.xyzw = 0.0f;
#endif
	
	lDifColor = g_Base.DiffuseSource  > 0.5f ? VSInput.Diffuse  : g_Common.Material.Diffuse ;
	
#if USE_SPE
	lSpeColor = g_Base.SpecularSource > 0.5f ? VSInput.Specular : g_Common.Material.Specular ;
#endif
	...
	// ライトの処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 開始 )
				...
				VSOutput.V_to_Eye_ShadowMap2PosZ.xyz = normalize( lBumpTemp ) ;
			#endif
			...
			lTotalSpecular = float4( 0, 0, 0, 0 ) ;
					...
					VSOutput.V_to_Eye_ShadowMap2PosZ.xyz = normalize( -lViewPosition.xyz ) ;
				#endif
...
// ライト2の処理 ***********************************************************************************************************************************( 終了 )
	...
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.xyz = VSOutput.Position.xyz ;
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.w   = 1.0f ;
	...
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.x = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 0 ][ 0 ] ) ;
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.y = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 0 ][ 1 ] ) ;
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.z = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 0 ][ 2 ] ) ;

	// ワールド座標をシャドウマップ１のライト設定の射影座標に変換
	VSOutput.ShadowMap1Pos_ShadowMap2PosY.x = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 1 ][ 0 ] ) ;
	VSOutput.ShadowMap1Pos_ShadowMap2PosY.y = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 1 ][ 1 ] ) ;
	VSOutput.ShadowMap1Pos_ShadowMap2PosY.z = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 1 ][ 2 ] ) ;

	// ワールド座標をシャドウマップ２のライト設定の射影座標に変換
	VSOutput.ShadowMap0Pos_ShadowMap2PosX.w = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 2 ][ 0 ] ) ;
	VSOutput.ShadowMap1Pos_ShadowMap2PosY.w = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 2 ][ 1 ] ) ;
	VSOutput.V_to_Eye_ShadowMap2PosZ.w      = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 2 ][ 2 ] ) ;
