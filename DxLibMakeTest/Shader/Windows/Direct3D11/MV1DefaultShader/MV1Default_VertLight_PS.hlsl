...
struct PS_INPUT
{
	...
	float4 V_to_Eye_ShadowMap2PosZ : TEXCOORD1 ;	// 頂点座標から視線へのベクトル( ビュー空間 )とシャドウマップ２のライトZ座標( w )
#endif
	...
	float4 ShadowMap0Pos_ShadowMap2PosX   : TEXCOORD6 ;	// シャドウマップ０のライト座標( x, y, z )とシャドウマップ２のライトX座標( w )
#endif
#if SHADOWMAP
	float4 ShadowMap1Pos_ShadowMap2PosY   : TEXCOORD7 ;	// シャドウマップ１のライト座標( x, y, z )とシャドウマップ２のライトY座標( w )
#endif
} ;
...
//	float4		variance_md_p_pmax ;
	...
	// ライトを使わない場合はカラーをそのまま出力 *****************************************( 開始 )
		...
		// シャドウマップへの描画 *********************************************************( 開始 )
			...
			// Ｚ値を色として出力
			PSOutput.Color0.r = PSInput.ShadowMap0Pos_ShadowMap2PosX.z;
//			PSOutput.Color0.g = PSInput.ShadowMap0Pos_ShadowMap2PosX.z * PSInput.ShadowMap0Pos_ShadowMap2PosX.z + 0.25f * ( ddx( PSInput.ShadowMap0Pos_ShadowMap2PosX.z ) * ddx( PSInput.ShadowMap0Pos_ShadowMap2PosX.z ) + ddy( PSInput.ShadowMap0Pos_ShadowMap2PosX.z ) * ddy( PSInput.ShadowMap0Pos_ShadowMap2PosX.z ) ) ;
	...
	// ライトを使う場合 *******************************************************************( 開始 )
		...
		// バンプマップ or フォンシェーディングの場合 =========================================( 開始 )
				...
				if( PSInput.ShadowMap0Pos_ShadowMap2PosX.x < -1.0f || PSInput.ShadowMap0Pos_ShadowMap2PosX.x > 1.0f ||
					PSInput.ShadowMap0Pos_ShadowMap2PosX.y < -1.0f || PSInput.ShadowMap0Pos_ShadowMap2PosX.y > 1.0f ||
					PSInput.ShadowMap0Pos_ShadowMap2PosX.z <  0.0f || PSInput.ShadowMap0Pos_ShadowMap2PosX.z > 1.0f )
				{
					ShadowRate.x = 1.0f;
				}
				else
				{
					// 深度テクスチャの座標を算出
					// PSInput.ShadowMap0Pos_ShadowMap2PosX.xy は -1.0f ～ 1.0f の値なので、これを 0.0f ～ 1.0f の値にする
					DepthTexCoord.x = ( PSInput.ShadowMap0Pos_ShadowMap2PosX.x + 1.0f ) / 2.0f;

					// yは更に上下反転
					DepthTexCoord.y = 1.0f - ( PSInput.ShadowMap0Pos_ShadowMap2PosX.y + 1.0f ) / 2.0f;
					...
					ShadowRate.x = smoothstep( PSInput.ShadowMap0Pos_ShadowMap2PosX.z - g_ShadowMap.Data[ 0 ].GradationParam, PSInput.ShadowMap0Pos_ShadowMap2PosX.z, TextureDepth.r + g_ShadowMap.Data[ 0 ].AdjustDepth ) ;
				}

				// 頂点のテクスチャ座標値が範囲内の場合のみ処理する
				if( PSInput.ShadowMap1Pos_ShadowMap2PosY.x < -1.0f || PSInput.ShadowMap1Pos_ShadowMap2PosY.x > 1.0f ||
					PSInput.ShadowMap1Pos_ShadowMap2PosY.y < -1.0f || PSInput.ShadowMap1Pos_ShadowMap2PosY.y > 1.0f ||
					PSInput.ShadowMap1Pos_ShadowMap2PosY.z <  0.0f || PSInput.ShadowMap1Pos_ShadowMap2PosY.z > 1.0f )
				{
					ShadowRate.y = 1.0f;
				}
				else
				{
					// 深度テクスチャの座標を算出
					// PSInput.ShadowMap1Pos_ShadowMap2PosY.xy は -1.0f ～ 1.0f の値なので、これを 0.0f ～ 1.0f の値にする
					DepthTexCoord.x = ( PSInput.ShadowMap1Pos_ShadowMap2PosY.x + 1.0f ) / 2.0f;

					// yは更に上下反転
					DepthTexCoord.y = 1.0f - ( PSInput.ShadowMap1Pos_ShadowMap2PosY.y + 1.0f ) / 2.0f;

					// 深度バッファテクスチャから深度を取得
					TextureDepth = g_ShadowMap1Texture.Sample( g_ShadowMap1Sampler, DepthTexCoord );

					// テクスチャに記録されている深度( +補正値 )よりＺ値が大きかったら奥にあるということで減衰率を最大にする
					ShadowRate.y = smoothstep( PSInput.ShadowMap1Pos_ShadowMap2PosY.z - g_ShadowMap.Data[ 1 ].GradationParam, PSInput.ShadowMap1Pos_ShadowMap2PosY.z, TextureDepth.r + g_ShadowMap.Data[ 1 ].AdjustDepth ) ;
				}

				// 頂点のテクスチャ座標値が範囲内の場合のみ処理する
				if( PSInput.ShadowMap0Pos_ShadowMap2PosX.w < -1.0f || PSInput.ShadowMap0Pos_ShadowMap2PosX.w > 1.0f ||
					PSInput.ShadowMap1Pos_ShadowMap2PosY.w < -1.0f || PSInput.ShadowMap1Pos_ShadowMap2PosY.w > 1.0f ||
					PSInput.V_to_Eye_ShadowMap2PosZ.w      <  0.0f || PSInput.V_to_Eye_ShadowMap2PosZ.w      > 1.0f )
				{
					ShadowRate.z = 1.0f;
				}
				else
				{
					// 深度テクスチャの座標を算出
					// PSInput.ShadowMap0Pos_ShadowMap2PosX.w と PSInput.ShadowMap1Pos_ShadowMap2PosY.w は -1.0f ～ 1.0f の値なので、これを 0.0f ～ 1.0f の値にする
					DepthTexCoord.x = ( PSInput.ShadowMap0Pos_ShadowMap2PosX.w + 1.0f ) / 2.0f;

					// yは更に上下反転
					DepthTexCoord.y = 1.0f - ( PSInput.ShadowMap1Pos_ShadowMap2PosY.w + 1.0f ) / 2.0f;

					// 深度バッファテクスチャから深度を取得
					TextureDepth = g_ShadowMap2Texture.Sample( g_ShadowMap2Sampler, DepthTexCoord );

					// テクスチャに記録されている深度( +補正値 )よりＺ値が大きかったら奥にあるということで減衰率を最大にする
					ShadowRate.z = smoothstep( PSInput.V_to_Eye_ShadowMap2PosZ.w - g_ShadowMap.Data[ 2 ].GradationParam, PSInput.V_to_Eye_ShadowMap2PosZ.w, TextureDepth.r + g_ShadowMap.Data[ 2 ].AdjustDepth ) ;
				}
...
// ライト0の処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 開始 )
	...
	// スペキュラカラー計算
		...
		// ハーフベクトルの計算
		TempF3 = PSInput.V_to_Eye_ShadowMap2PosZ.xyz + PSInput.Light0_Dir_Gen.xyz ;
		#ifdef    PHONG
...
// ライト1の処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 開始 )
	...
	// スペキュラカラー計算
		...
		// ハーフベクトルの計算
		TempF3 = PSInput.V_to_Eye_ShadowMap2PosZ.xyz + PSInput.Light1_Dir_Gen.xyz ;
		#ifdef    PHONG
			TempF3 = normalize( TempF3 ) ;
		#else  // PHONG
			TempF3 *= 0.5f ;
...
// ライト2の処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 開始 )
	...
	// スペキュラカラー計算
		...
		// ハーフベクトルの計算
		TempF3 = PSInput.V_to_Eye_ShadowMap2PosZ.xyz + PSInput.Light2_Dir_Gen.xyz ;
		#ifdef    PHONG
			TempF3 = normalize( TempF3 ) ;
		#else  // PHONG
			TempF3 *= 0.5f ;
