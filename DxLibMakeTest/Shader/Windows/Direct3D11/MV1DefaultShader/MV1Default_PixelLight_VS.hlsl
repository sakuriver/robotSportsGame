...
VS_OUTPUT main( VS_INPUT VSInput )
{
	...
	// スペキュラカラーをセット
	VSOutput.Specular = g_Base.SpecularSource > 0.5f ? VSInput.Specular : g_Common.Material.Specular ;
	...
	// フォグ計算 =============================================( 開始 )
			...
			// 線形フォグ計算
			VSOutput.Fog_ShadowMap2PosZ.x = lViewPosition.z * g_Common.Fog.LinearDiv + g_Common.Fog.LinearAdd ;
			...
			VSOutput.Fog_ShadowMap2PosZ.x = 1.0f / pow( abs( g_Common.Fog.E ), lViewPosition.z * g_Common.Fog.Density ) ;
			...
			VSOutput.Fog_ShadowMap2PosZ.x = 1.0f / pow( abs( g_Common.Fog.E ), ( lViewPosition.z * g_Common.Fog.Density ) * ( lViewPosition.z * g_Common.Fog.Density ) ) ;

		#endif

	#else // FOG_LINEAR || FOG_EXP || FOG_EXP2
	
		VSOutput.Fog_ShadowMap2PosZ.x = 1.0f;
	...
	VSOutput.Fog_ShadowMap2PosZ.y           = dot( lWorldPosition, g_OtherMatrix.ShadowMapLightViewProjectionMatrix[ 2 ][ 2 ] ) ;

	// 深度影用のライトから見た射影座標を算出 =================( 終了 )
#else

	VSOutput.Fog_ShadowMap2PosZ.y = 0.0f;
