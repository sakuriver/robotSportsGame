...
//			if( WinAPIData.Win32Func.joyGetPosExFunc( i, &joyex ) != JOYERR_NOERROR ) break ;
			...
			InputSysData.Pad[ i ].Buttons = joycaps.wNumButtons ;
		}
...
extern int RefreshEffectPlayState_PF( void )
{
	...
	{
		// ここでジョイパッドの接続に変化があったかをチェックする
//		int PadInstanceGUIDNum = InputSysData.PF.PadInstanceGUIDNum ;
//		InputSysData.PF.PadInstanceGUIDNum = 0 ;
//		InputSysData.PF.DirectInputObject->EnumDevices( D_DIDEVTYPE_JOYSTICK, EnumJoypadCheckCallback, NULL, D_DIEDFL_ATTACHEDONLY ) ;
//		if( PadInstanceGUIDNum != InputSysData.PF.PadInstanceGUIDNum )
//		{
//			// 変化があった場合はジョイパッドの最初期化を行う
//			SetupJoypad() ;
//
//			// パッドデバイスの再取得
//			num  = InputSysData.PadNum ;
