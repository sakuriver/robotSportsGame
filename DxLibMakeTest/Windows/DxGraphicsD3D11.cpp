...
extern	int		Graphics_D3D11_Initialize( void )
{
	...
	if( GetWindowModeFlag() == FALSE || IsUseSubBackBuffer )
	{
...
static	int		Graphics_D3D11_OutputWindow_CreateSwapChain( int Index )
{
	...
	if( GAPIWin.DXGIFactory2Object != NULL )
	{
		...
		// スワップチェインの作成
		{
			...
			if( OWI->IsMainTarget )
			{
				OWI->IsFullscreen = NS_GetWindowModeFlag() == FALSE ? TRUE : FALSE ;
				if( OWI->IsFullscreen == FALSE )
				{
					SwapChainDesc1.Width			= ( UINT )GSYS.Screen.MainScreenSizeX ;
					SwapChainDesc1.Height			= ( UINT )GSYS.Screen.MainScreenSizeY ;
					FullScreenDesc.Windowed			= TRUE ;

					DXST_LOGFILEFMT_ADDUTF16LE(( "\x5b\x00\xa6\x30\xa4\x30\xf3\x30\xc9\x30\xa6\x30\xe2\x30\xfc\x30\xc9\x30\x20\x00\x25\x00\x64\x00\x78\x00\x25\x00\x64\x00\x5d\x00\x00"/*@ L"[ウインドウモード %dx%d]" @*/, SwapChainDesc1.Width, SwapChainDesc1.Height )) ;
				}
				else
				{
...
extern	int		Graphics_D3D11_Device_Initialize( void )
{
	...
	// 描画ステータスのセット
	{
		...
		// ブレンドモード設定
		Graphics_D3D11_DeviceState_SetBlendMode( DX_BLENDMODE_NOBLEND, FALSE ) ;
...
extern int EnumDXGIAdapterInfo( void )
{
	...
	for( GD3D11.Adapter.InfoNum = 0 ; GD3D11.Adapter.InfoNum < DX_D3D11_MAX_ADAPTER_LISTUP ; GD3D11.Adapter.InfoNum ++, AdapterInfo ++ )
	{
		...
		DXST_LOGFILEFMT_ADDW(( L"	Adapter No.%d Desc:%s", GraphicsHardDataDirect3D11.Adapter.InfoNum, AdapterInfo->Desc.Description )) ;
		...
		for( AdapterInfo->OutputNum = 0 ; AdapterInfo->OutputNum < DX_D3D11_MAX_OUTPUT_LISTUP ; AdapterInfo->OutputNum ++ )
		{
			...
			DXST_LOGFILEFMT_ADDW(( L"		Output Device No.%d Name:%s", AdapterInfo->OutputNum, AdapterInfo->OutputDesc[ AdapterInfo->OutputNum ].DeviceName )) ;
...
// 描画ブレンドモードのセット
extern	int		Graphics_D3D11_DeviceState_SetBlendMode( int BlendMode, int NotWriteAlphaChannelFlag )
{
	if( GAPIWin.D3D11DeviceObject == NULL )
	{
		return -1 ;
...
extern	int		Graphics_D3D11_DeviceState_NormalDrawSetup( void )
{
	...
	{
		Graphics_D3D11_DeviceState_SetBlendMode( NextBlendMode, GD3D11.Device.DrawSetting.NotWriteAlphaChannelFlag ) ;
	}
...
extern void Graphics_D3D11_DrawPreparationToShader( int ParamFlag, int Is2D )
{
	...
	// 出力先とのブレンド方式をセット
	Graphics_D3D11_DeviceState_SetBlendMode( GSYS.DrawSetting.BlendMode == DX_BLENDMODE_SUB ? DX_BLENDMODE_SUB1 : GSYS.DrawSetting.BlendMode, GSYS.DrawSetting.NotWriteAlphaChannelFlag ) ;
...
extern int NS_SetUseDirect3D11AdapterIndex( int Index )
{
	...
	{
		if( CL_strcmp( DX_CHARCODEFORMAT_UTF16LE, ( char * )GD3D11.Adapter.Info[ Index ].OutputDesc[ 0 ].DeviceName, ( char * )GSYS.Screen.DisplayInfo[ i ].Name ) == 0 )
		{
